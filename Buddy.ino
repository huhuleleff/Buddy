 ...
 if (WiFi.status() != WL_CONNECTED && poiskusipovezati) {
  ...
  if (otaPendingFromWeb && defaultFirmwareUrl.length() > 0) {
    if (millis() - otaWebRequestTime >= OTA_WEB_DELAY) {
      ...
      unsigned long remaining = (OTA_WEB_DELAY - (millis() - otaWebRequestTime)) / 1000;
      if (remaining % 5 == 0) { // Only print every 5 seconds to avoid spam
        Serial.println("[UPDATE] OTA starting in " + String(remaining) + " seconds...");
      }
    }
  }
