...
#define DATA_PIN          15    // GPIO21 - Shared data line
#define DISCOVER_PIN      16    // GPIO22 - Discovery chain control
...
// Forward declarations for OTA display functions
void displayOTAScreen(String status, int progress = 0, String details = "");
void displayOTADownloadProgress(int progress, size_t downloaded, size_t total);
void displayOTAInstallProgress(int progress, size_t written, size_t total);
void displayOTASuccess();
void displayOTAError(String error);

// OTA Firmware Download Function
bool downloadFirmware(String url, String filename) {
  ...
  displayOTAScreen("Starting download...", 0, "Connecting to: " + url.substring(0, 30) + "...");
  
  // Manual redirect handling for older ESP32 cores
  while (redirectCount < maxRedirects) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      break; // Success, no redirect needed
      ...
      if (redirectUrl.length() == 0) {
        Serial.println("Redirect received but no location header found");
        return false;
      }
      ...
      if (redirectCount >= maxRedirects) {
        Serial.println("Maximum redirect limit reached");
        return false;
      }
    } else {
      // Other HTTP error
      ...
      // Additional debugging for GitHub URLs
      if (url.indexOf("github.com") != -1) {
        Serial.println("GitHub URL detected - ensure you're using raw file URL");
        Serial.println("Correct format: https://github.com/user/repo/releases/download/v1.0/firmware.bin");
      }
      http.end();
      return false;
    }
  ...
  // If we got here, we have a successful response (HTTP_CODE_OK)
  int contentLength = http.getSize();
  Serial.printf("Firmware size: %d bytes\n", contentLength);
  
  // Check available space
  size_t freeSpace = FFat.freeBytes();
  if (contentLength > freeSpace) {
    Serial.println("Not enough space on FATFS");
    http.end();
    return false;
  }
  ...
  if (file) {
    ...
    while (http.connected() && bytesRead < contentLength) {
      ...
      if (size) {
        int read = stream->readBytes(buffer, min((size_t)size, sizeof(buffer)));
        file.write(buffer, read);
        bytesRead += read;
        
        // Feed watchdog to prevent timeout during long downloads
        esp_task_wdt_reset();
        yield(); // Allow other tasks to run
        
        // Send progress via WebSocket every 10%
        int progress = (bytesRead * 100) / contentLength;
        if (progress - lastProgress >= 10) {
          webSocket.textAll("{\"ota_progress\":" + String(progress) + "}");
          lastProgress = progress;
          Serial.printf("Download progress: %d%%\n", progress);
          
          // Update display every 10%
          displayOTADownloadProgress(progress, bytesRead, contentLength);
        }
      }
      delay(1);
      
      // Additional watchdog reset every loop iteration
      esp_task_wdt_reset();
    }
    file.close();
    http.end();
    ...
    if (bytesRead == contentLength) {
      ...
      displayOTAScreen("Download complete!", 100, "Preparing installation...");
      return true;
    } else {
      Serial.println("Download incomplete");
      displayOTAError("Download incomplete");
      return false;
    }
  }
...
// Perform local OTA update - extracted from working local OTA endpoint
void performLocalOTAUpdate(String filename) {
  String fullPath = "/" + filename;
  Serial.println("[OTA] Requested update for file: " + filename);
  
  // Initialize display for installation
  displayOTAScreen("Preparing install...", 0, "File: " + filename);
      ...
      if (Update.begin(firmwareSize)) {
        ...
        while (firmwareFile.available()) {
          ...
          if (written % 100000 == 0) {
            ...
            displayOTAInstallProgress(progress, written, firmwareSize);
          }
        }
        ...
        if (written == firmwareSize && Update.end(true)) {
          ...
          // Show error screen
          displayOTAError("Installation failed");
        }
      } else {
        Serial.println("[OTA] Not enough space for update");
        webSocket.textAll("{\"ota_status\":\"no_space\"}");
        displayOTAError("Not enough space");
      }
    } else {
      Serial.println("[OTA] Failed to open firmware file");
      webSocket.textAll("{\"ota_status\":\"file_not_found\"}");
      displayOTAError("File not found");
    }
  } else {
...
void displayOTAScreen(String status, int progress = 0, String details = "") {
  ...
  tft.setTextSize(1);
  tft.setCursor(10, 60);
  tft.println("Status: " + status);
  
  if (progress > 0) {
    tft.setCursor(10, 80);
    tft.println("Progress: " + String(progress) + "%");
    
    // Draw progress bar
    tft.drawRect(10, 100, 300, 20, TFT_WHITE);
    int barWidth = (300 * progress) / 100;
    if (barWidth > 0) {
      tft.fillRect(11, 101, barWidth - 1, 18, TFT_GREEN);
    }
  }
  ...
  if (details.length() > 0) {
    ...
    if (details.length() > 30) {
      tft.println(details.substring(0, 30));
      tft.setCursor(10, 160);
      tft.println(details.substring(30));
    } else {
      tft.println(details);
    }
  }
  ...
  tft.println("Time: " + String(ure) + ":" + String(minuta) + ":" + String(sekunde));
}

void displayOTADownloadProgress(int progress, size_t downloaded, size_t total) {
  displayOTAScreen("Downloading...", progress, 
    "Downloaded: " + String(downloaded / 1024) + "KB / " + String(total / 1024) + "KB");
}

void displayOTAInstallProgress(int progress, size_t written, size_t total) {
  displayOTAScreen("Installing...", progress,
    "Written: " + String(written / 1024) + "KB / " + String(total / 1024) + "KB");
}

void displayOTASuccess() {
  displayOTAScreen("SUCCESS!", 100, "Update completed. Rebooting...");
  delay(2000);
}
...
void displayOTAError(String error) {
  ...
  tft.println("Please check firmware file");
  ...
  server.on("/ota/download", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (request->hasParam("url", true)) {
      ...
      if (downloadFirmware(url, filename)) {
        ...
        // Trigger the existing local OTA function that already works
        request->send(200, "text/plain", "Download complete. Starting OTA update...");
        
        // Call the local OTA update function directly
        performLocalOTAUpdate(filename);
      } else {
        webSocket.textAll("{\"ota_status\":\"download_failed\"}");
        request->send(500, "text/plain", "Download failed");
      }
    } else {
...
void loop() {
 ...
 // Update time from RTC every loop iteration
  ...
  if (millis() - lastDebugPrint > 5000) {  // Print every 5 seconds
    ...
    Serial.printf("RTC Time: %02d:%02d:%02d %02d.%02d.%d\n", 
                  ure, minuta, sekunde, dnevi, mesec, leto);
  }
...
void izrismreze() {
  ...
  if (flaggrafmoci) { tft.pushImage(0, 0, animation_width, animation_height, SheetFan[0]); }
}
